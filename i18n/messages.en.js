module.exports = {
  ...
  messages: {
    ...
    header_course_changes_comment: 'Changes of the course before this course offering (max 2000 characters)',
    ...
    alert_over_max_fields: {
      alterationText: 'Summarize changes field contains too many characters.',
      default: 'Field contains too many characters',
    },
    ...
    info_examination_grade: {
      ...
      body: 'Graduation rate is defined as the number of passed first registration students on the whole course by entered end date divided by the number of registered students (as defined above) on the course offering in percent (%). The number is fetched automatically from Ladok for course offerings with a start semester from Spring 2017 and later based on the end date of the course offering. For some courses, the examination date is after the end of the course, which means that the graduations rate will be 0. Therefore it is possible to manually enter the end date to let the system recalculate the graduation rate. It is also possible to enter the graduation rate manually. In that case, the end date is not relevant. The graduation rate is not calculated for each examination session in the course examination set. Neither is it recalculated after each following re-examination. ',
      btnCancel: 'Close',
    },
